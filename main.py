...
def iniciar_cli():
    ...
    while True:
        ...
        print("3. Registrar Gasto Regular")
        print("4. Registrar Gasto de Ocio")
        print("5. Ejecutar Cobro de Suscripción (Evaluar Tablas de Decisión)")
        print("6. Ejecutar Corte de Caja y Salir")
        ...
        try:
            if opcion == '1':
                ...
                print(f"\nTienes ${guardian.suscripciones_pendientes} en suscripciones.")
                monto_cobro = float(input("¿Cuánto vas a procesar ahora?: $"))

                es_vip = input("¿El usuario es VIP? (s/n): ").lower() == 's'
                cuenta_activa = input("¿La cuenta está activa? (s/n): ").lower() == 's'
                tarjeta_vencida = input("¿La tarjeta está vencida? (s/n): ").lower() == 's'
                ...
                print(f"Alertas generadas en el mes: {len(reporte['alertas_mes'])}")
